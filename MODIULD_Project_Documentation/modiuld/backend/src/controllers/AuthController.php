...
class AuthController {
    ...
    public static function googleConfig(): void {
        ...
        if ($clientId === '') { self::json(['error' => 'Google Sign-In has not been configured.'], 503); return; }
        self::json(['client_id' => $clientId]);
    }
    ...
    public static function googleSignIn(): void {
        ...
        $clientId = getenv('GOOGLE_CLIENT_ID') ?: '';
        if (!$idToken || !$clientId) { self::json(['error' => 'Google Sign-In is not configured.'], 503); return; }

        $client = new Google_Client(['client_id' => $clientId]);
        try { $google = $client->verifyIdToken($idToken); } catch (Throwable $e) { $google = false; }
        if (!$google || empty($google['sub']) || empty($google['email']) || empty($google['email_verified'])) {
            self::json(['error' => 'Google could not verify this account.'], 401); return;
        }

        $db = Database::getConnection();
        $stmt = $db->prepare('SELECT id, email, username, display_name FROM users WHERE google_sub = ?');
        $stmt->execute([$google['sub']]);
        $user = $stmt->fetch();
        if ($user) { self::issueToken($user); return; }

        // Do not silently attach Google access to an existing password account.
        $stmt = $db->prepare('SELECT id FROM users WHERE email = ?');
        $stmt->execute([$google['email']]);
        if ($stmt->fetch()) { self::json(['error' => 'This email already has an account. Sign in with your password first.'], 409); return; }

        $pending = JwtHelper::encode(['purpose' => 'google_signup', 'google_sub' => $google['sub'], 'email' => $google['email'], 'name' => $google['name'] ?? '', 'exp' => time() + 600]);
        self::json(['needs_username' => true, 'signup_token' => $pending, 'email' => $google['email']]);
    }

    public static function googleComplete(): void {
        $data = json_decode(file_get_contents('php://input'), true);
        $pending = JwtHelper::decode(trim($data['signup_token'] ?? ''));
        $username = trim($data['username'] ?? '');
        if (!$pending || ($pending['purpose'] ?? '') !== 'google_signup') { self::json(['error' => 'Your Google sign-up session has expired. Please try again.'], 401); return; }
        if (!preg_match('/^[a-zA-Z0-9_]{3,20}$/', $username)) { self::json(['error' => 'Username must be 3-20 characters long and contain only letters, numbers, and underscores.'], 422); return; }

        $db = Database::getConnection();
        $stmt = $db->prepare('SELECT id FROM users WHERE username = ?');
        $stmt->execute([$username]);
        if ($stmt->fetch()) { self::json(['error' => 'This username is already taken.'], 409); return; }
        $stmt = $db->prepare('SELECT id, email, username, display_name FROM users WHERE google_sub = ?');
        $stmt->execute([$pending['google_sub']]);
        if ($user = $stmt->fetch()) { self::issueToken($user); return; }
        try {
            $displayName = trim($pending['name'] ?? '') ?: $username;
            $ins = $db->prepare('INSERT INTO users (email, username, password_hash, google_sub, display_name) VALUES (?, ?, NULL, ?, ?)');
            $ins->execute([$pending['email'], $username, $pending['google_sub'], $displayName]);
            self::issueToken(['id' => $db->lastInsertId(), 'email' => $pending['email'], 'username' => $username, 'display_name' => $displayName], 201);
        } catch (PDOException $e) { self::json(['error' => 'This Google account or username is already in use.'], 409); }
    }

    private static function issueToken(array $user, int $status = 200): void {
        $token = JwtHelper::encode(['sub' => $user['id'], 'email' => $user['email'], 'username' => $user['username']]);
        self::json(['token' => $token, 'user' => ['id' => $user['id'], 'email' => $user['email'], 'username' => $user['username'], 'display_name' => $user['display_name']]], $status);
    }
    ...
    public static function changePassword(): void {
        ...
        $stmt->execute([$payload['sub']]);
        $user = $stmt->fetch();
        if (!$user || !$user['password_hash'] || !password_verify($oldPass, $user['password_hash'])) {
